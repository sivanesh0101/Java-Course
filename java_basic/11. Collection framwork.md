# Java Collections Framework — Beginner-Friendly Guide

A super simple guide for freshers to understand **what** each collection is, **why** we use it, and **where** it fits — with tiny examples.
...
- **Add/remove at ends a lot?** → `LinkedList`
- **Need uniqueness (no duplicates)?** → `HashSet`
- **Need uniqueness + sorted order?** → `TreeSet`
- **Look up value by key fast?** → `HashMap`
...
## Two Golden Rules for Freshers
1. If unsure, start with **`ArrayList`** (for sequences) or **`HashMap`** (for key→value).  
2. If you need **uniqueness**, use a **Set** (`HashSet` if order doesn’t matter, `TreeSet` if you want sorted).

---

Happy learning! Try copying each snippet into its own `Demo*.java` file and run it to see the output.